    ...
    JSJ_Gallery_SlideShow_Utilities = function() {
        ...
        self.update_pagination_string = function($el, slideNumber) {
            ...
            if (slideNumber !== undefined && slideNumber > 0) {
                string = "(" + (slideNumber + 1) + " " + __self.of_string + " " + total + ")";
            } else {
                string = "(" + 1 + " " + __self.of_string + " " + total + ")";
            }
            $("#galleryNumbering-" + galleryId).html(string);
        };
...
(function($) {
    ...
    window.createJSJGallerySlideshow = function() {
        var settings = window.jsjGallerySlideshowOptions.settings, cycleGallery = [], utilities = new window.JSJ_Gallery_SlideShow_Utilities();
        ...
        return function() {
            $($.fn.cycle.defaults.autoSelector).each(function(index) {
                var $this = $(this), gallery_id = $this.attr("id").replace("galleryid-", "");
                utilities.update_pagination_string(gallery_id);
                $this.parent().children(".gallery-pager").html("");
                cycleGallery[index] = $this.cycle("destroy").cycle(settings);
